...
export class OpeningTimesService {
  ...
  get(id: number): Observable<any> {
    ...
    return this.http
      .get(this.url + 'openingtime', options)
  ...
  postOpeningTime(model: any): Observable<any> {
    //let body = JSON.stringify(model);
    //let headers = new Headers({ 'Content-Type': 'application/json' });
    //let options = new RequestOptions({ headers: headers });
    return this.http
      .post(this.url + 'openingtime', model)
      .catch(this.handleError);
  }
  putOpeningTime(model: any): Observable<any> {
    let body = JSON.stringify(model);
    let headers = new Headers({ 'Content-Type': 'application/json' });
    let options = new RequestOptions({ headers: headers });
    return this.http
      .put(this.url + 'openingtime', body, options)
      .catch(this.handleError);
  }
