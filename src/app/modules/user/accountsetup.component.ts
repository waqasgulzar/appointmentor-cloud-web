...
export class AccountSetupComponent implements OnInit {
  ...
  ngOnInit() {
    this.userForm = this.fb.group({
      ...
      phoneNumber: ['', Validators.compose([Validators.required, Validators.maxLength(12), Validators.pattern("[0-9]{0-10}")])],
  ...
  LoadUser() {
    this.userService.getEmailAddress(apiUrl, Number(sessionStorage.getItem("organizationId"))).subscribe((data: any) => {
  ...
  onSubmit(userForm: FormGroup) {
    this.submitted = true;
    if (!userForm.invalid) {
      this.userService.put(apiUrl, Number(sessionStorage.getItem("organizationId")), userForm.value).subscribe((data: any) => {
